...
// Estrutura para armazenar os bitmaps
typedef struct {
	//SOLDADO
	ALLEGRO_BITMAP* walkright; ALLEGRO_BITMAP* walkleft; ALLEGRO_BITMAP* jump_image; ALLEGRO_BITMAP* idle_right; ALLEGRO_BITMAP* idle_left; ALLEGRO_BITMAP* shot_image_right;
	ALLEGRO_BITMAP* shot_image_left; ALLEGRO_BITMAP* pegar_image; ALLEGRO_BITMAP* tronco3; ALLEGRO_SAMPLE* tiro;
	//ARMAS
	ALLEGRO_BITMAP* bullet_right; ALLEGRO_BITMAP* bullet_left;
	//AUDIO
	//ALLEGRO_SAMPLE* tiro2;

} Recursos;

bool carregar_bitmaps(Recursos* recursos) {
	...
	recursos->tronco3 = al_load_bitmap("./Obstaculos/wood_spike.png");
	//recursos->tiro = al_load_sample("./Audios/metralhadora.wav");
	...
	// Verificar se todos os bitmaps foram carregados com sucesso
	if (!recursos->walkright || !recursos->walkleft || !recursos->jump_image || !recursos->idle_right || !recursos->idle_left ||
		!recursos->shot_image_right || !recursos->shot_image_left || !recursos->pegar_image|| !recursos->tronco3  ||
		!recursos->tiro|| !recursos->bullet_right || !recursos->bullet_left) {
		return false;
	}
	return true;
}
...
void liberar_recursos(Recursos* recursos) {
	...
	if (recursos->tronco3) al_destroy_bitmap(recursos->tronco3);
...
int tela4() {
	...
	Recursos recursos;
	if (!carregar_bitmaps(&recursos)) {
		fprintf(stderr, "Falha ao carregar bitmaps.\n");
		return -1;
	}
	...
	int posicao = 0, pulo = 0, velocy = 0, velocup = 15, plimit = 0;
	float frame = 0.f, frame2 = 0.f, frame4px = 0.f, frameidle = 0.f, frameright = 0.f;
	int pos_x = 0, pos_y = 380;
	int current_frame_y = 0;
	bool moving_left = false, moving_up = false, moving_down = false, moving_right = false, mov = false, atirando = false;
	float pos_x_bullet = 0.1;
	//VariÃ¡veis dos consumÃ­veis
	int kitmedicoX = 600, kitmunicaoX = 680;

	//TEMPORIZADOR
	float temporizador = 2.0, tempo_decorrido = 0.0; 

	//colisão
	static bool inicializado = false,  bala_ativa = false;
	...
	while (true) {
		...
		else  if (mov == false) jump_state = IDLE_RIGHT;
		...
		frame2 += 0.7f;
		if (frame2 > 7) frame2 -= 7;

		frame4px += 0.4f;
		if (frame4px > 4) frame4px -= 4;

		frameidle += 0.7f;
		if (frameidle > 7) frameidle -= 7;

		switch (jump_state) {
		case WALKING_RIGHT: al_draw_bitmap_region(recursos.walkright, 256 * (int)frame, current_frame_y, 256, 256, pos_x, pos_y, 0);
			break;

		case WALKING_LEFT: al_draw_bitmap_region(recursos.walkleft, 256 * (int)frame, current_frame_y, 256, 256, pos_x, pos_y, 0);
			break;

		case WALKING_SHOT_RIGHT: al_draw_bitmap_region(recursos.shot_image_right, 256 * (int)frame4px, current_frame_y, 256, 256, pos_x, pos_y, 0);
			break;

		case WALKING_SHOT_LEFT: al_draw_bitmap_region(recursos.shot_image_left, 256 * (int)frame4px, current_frame_y, 256, 256, pos_x, pos_y, 0);
			break;

		case JUMPING:if (moving_left) {
			al_draw_bitmap_region(recursos.jump_image, 256 * (int)frame, current_frame_y, 256, 256, pos_x, pos_y, ALLEGRO_FLIP_HORIZONTAL);
			mov = true;
			//Inverter imagem do pulo
		}
			else {
			al_draw_bitmap_region(recursos.jump_image, 256 * (int)frame2, current_frame_y, 256, 256, pos_x, pos_y, 0);
			mov = true;

		}
			break;
		case PEGAR: al_draw_bitmap_region(recursos.pegar_image, 256 * (int)frame4px, current_frame_y, 256, 256, pos_x, pos_y, 0);
			break;

		case IDLE_LEFT: al_draw_bitmap_region(recursos.idle_left, 256 * (int)frameidle, current_frame_y, 256, 256, pos_x, pos_y, 0);
			break;

		case IDLE_RIGHT: al_draw_bitmap_region(recursos.idle_right, 256 * (int)frameidle, current_frame_y, 256, 256, pos_x, pos_y, 0);
			break;
		default:
			break;
		}
		...
		if (event.type == ALLEGRO_EVENT_TIMER) {
			...
			if (tempo_decorrido >= temporizador) {
				if (moving_left) jump_state = IDLE_LEFT;
		...
		case ALLEGRO_KEY_V:
			jump_state = PEGAR;
			if (pos_x > 465 && pos_x < 530 && pos_y + 128 > pos_y && pos_y < pos_y + 100) kitmedicoX = -1000;

			else if (pos_x > 560 && pos_x < 670 && pos_y + 128 > pos_y && pos_y < pos_y + 100) kitmunicaoX = -1000;
		...
		switch (event.keyboard.keycode) {
		case ALLEGRO_KEY_LEFT: moving_left = false;
			break;
		case ALLEGRO_KEY_RIGHT: moving_right = false;
			break;
		case ALLEGRO_KEY_UP: moving_up = false;
			break;
		case ALLEGRO_KEY_DOWN: moving_down = false;
			break;
		}
		...
		if (atirando) {
			...
			if (bala_ativa) {
				if (moving_left == false) {
					al_draw_bitmap(recursos.bullet_right, pos_x_bullet, pos_y + 167, 0);
					pos_x_bullet += 20.2f;
				}
				else {
					al_draw_bitmap(recursos.bullet_left, pos_x_bullet, pos_y + 167, 0);
					pos_x_bullet -= 20.2f;
				}
			}
		}
		else inicializado = false; // Reseta quando não está atirando
...
static int tela2() {
	...
	//int image1 = walkright;
	int posicao = 0, pulo = 0, velocy = 0, velocup = 15, plimit = 0;
	float frame7px = 0.f,  frame4px = 0.f;
	int pos_x = 0, pos_y = 380;
	int current_frame_y = 0;
	bool moving_left = false, mov = false, atirando = false;
	float pos_x_bullet = 0.1;

	//VARIÁVEIS INIMIGO
float frameinimigo = 0.f;
int enemy_pos_x = 1200, enemy_pos_y = 600;
...
int gen_pos_x = 900, gen_pos_y = 500;
...
GenState current_gen_state = GEN_RIGHT;
	...
	float frame_rate = 0.15f, tempo_animacao = 0.0, tempo_animacao2 = 0.0, tempo_animacao_enemy = 0.0;
	int max_frame = 7, max_frame2 = 4, max_frame_enemy = 3;

	//Variáveis das Estilizações alienigenas
	int EstAlien_pos_x = 200, EstAlien_pos_y = 200;

	//Tronco e colisão
	int TRONCO_X_MIN = 320, TRONCO_X_MAX = 350, troncoX = 400;
	static bool inicializado = false, bala_ativa = false, troncoVivo = true;
	typedef enum { WALKING_RIGHT, WALKING_LEFT, WALKING_SHOT_RIGHT, WALKING_SHOT_LEFT, JUMPING, IDLE_LEFT, IDLE_RIGHT, PEGAR
	}characterState;
	...
	while (true) {
		...
		if (pos_x + 75 > 320 && pos_x < 350 && pos_y + 128 > pos_y && pos_y < pos_y + 100) al_draw_text(font, al_map_rgb(0, 0, 0), 640, 360, ALLEGRO_ALIGN_CENTER, "Teste");
		...
		if (event.type == ALLEGRO_EVENT_TIMER) {
			...
			if (tempo_animacao >= frame_rate) {
				frame7px += 1.0f;
				if (frame7px >= max_frame) frame7px = 0; // Reinicia o quadro de animação
			...
			if (tempo_animacao2 >= frame_rate) {
				frame4px += 1.0f;
				if (frame4px >= max_frame2) frame4px = 0; // Reinicia o quadro de animação
			...
			if (tempo_animacao_enemy >= frame_rate) {
				frameinimigo += 1.0f;
				if (frameinimigo >= max_frame_enemy) frameinimigo = 0; // Reinicia o quadro de animação do inimigo
			}
			...
			//ObstÃ¡culos
			al_draw_bitmap(recursos.tronco3, 400, 516, 0);
			al_draw_bitmap(recursos.tronco3, troncoX, 516, 0);
			//Estilizações
			//Alien 1 x,y a
			...
			if (enemy_moving_right) current_enemy_state = INIMIGO_RIGHT;
			
			else current_enemy_state = INIMIGO_LEFT;

			//INIMIGO
			switch (current_enemy_state) {
			case INIMIGO_RIGHT: al_draw_bitmap_region(inimigoright, 61 * (int)frameinimigo, 0, 183, 64, enemy_pos_x, enemy_pos_y, 0);

			case INIMIGO_LEFT: al_draw_bitmap_region(inimigoleft, 61 * (int)frameinimigo, 0, 183, 64, enemy_pos_x, enemy_pos_y, 0);
				break;
			default:
				break;
			}
			...
			case GEN_RIGHT: al_draw_bitmap_region(generalright, 98 * (int)framegeneral, 0, 295, 102, gen_pos_x, gen_pos_y, 0);
				break;
			case GEN_LEFT: al_draw_bitmap_region(generalleft, 98 * (int)framegeneral, 0, 295, 102, gen_pos_x, gen_pos_y, 0);
				break;
			default:
				break;
			}
			...
			switch (jump_state) {
			case WALKING_RIGHT: al_draw_bitmap_region(recursos.walkright, 256 * (int)frame7px, current_frame_y, 256, 256, pos_x, pos_y, 0);
				break;

			case WALKING_LEFT: al_draw_bitmap_region(recursos.walkleft, 256 * (int)frame7px, current_frame_y, 256, 256, pos_x, pos_y, 0);
				break;
			...
			case JUMPING:if (moving_left) al_draw_bitmap_region(recursos.jump_image, 256 * (int)frame7px, current_frame_y, 256, 256, pos_x, pos_y, ALLEGRO_FLIP_HORIZONTAL);
				//Inverter imagem do pulo
			else al_draw_bitmap_region(recursos.jump_image, 256 * (int)frame7px, current_frame_y, 256, 256, pos_x, pos_y, 0);
			    break;

			case PEGAR: al_draw_bitmap_region(recursos.pegar_image, 256 * (int)frame4px, current_frame_y, 256, 256, pos_x, pos_y, 0);
				break;

			case IDLE_LEFT: al_draw_bitmap_region(recursos.idle_left, 256 * (int)frame7px, current_frame_y, 256, 256, pos_x, pos_y, 0);
				break;

			case IDLE_RIGHT: al_draw_bitmap_region(recursos.idle_right, 256 * (int)frame7px, current_frame_y, 256, 256, pos_x, pos_y, 0);
				break;
			default:
				break;
			}
...
static int tela1() {
	...
	int pulo = 0, velocy = 0,  velocup = 15, plimit = 0;
	float frame7px = 0.f, frame4px = 0.f;
	int pos_x = 0, pos_y = 380;
	int current_frame_y = 0;
	bool moving_left = false, mov = false, atirando = false;
	...
	//Variáveis das Estilizações alienigenas
	int EstAlien_pos_x = 60, EstAlien_pos_y = 0;

	//TEMPORIZADOR PERSONAGEM ANDANDO!!
	float frame_rate = 0.15f, tempo_animacao = 0.0, tempo_animacao2 = 0.0;
	int max_frame = 7, max_frame2 = 4;

	//Tronco e colisão
	int TRONCO_X_MIN = 320, TRONCO_X_MAX = 350, troncoX = 400;
	static bool inicializado = false, bala_ativa = false, troncoVivo = true,  shot_control = false;
	...
	while (true) {
		...
		if (event.type == ALLEGRO_EVENT_TIMER) {
			...
			case WALKING_SHOT_LEFT:
				al_draw_bitmap_region(recursos.shot_image_left, 256 * (int)frame4px, current_frame_y, 256, 256, pos_x, pos_y, 0);
				break;
	...
	while (true) {
		...
		if (event.type == ALLEGRO_EVENT_KEY_DOWN) {
			if (event.keyboard.keycode == ALLEGRO_KEY_UP) {
				controlador++;
				if (controlador > 4) controlador = 1;
			}
